...
class CollectionViewCell: UICollectionViewCell {
    ...
    lazy var backScroll : UIScrollView = {
        let scroll = UIScrollView.init(frame: CGRect(x: 0,y: 0, width: ScreenW, height: ScreenH))
...
//            self.backView.backgroundColor = UIColor.black.withAlphaComponent((ScreenH - CGFloat(alphaScale)) / ScreenH)
    ...
    public override func gestureRecognizerShouldBegin(_ gestureRecognizer: UIGestureRecognizer) -> Bool {
        ...
        // 正在缩放的view，不支持手势
        guard panView.frame.size.width == ScreenW else{
            return false
        }
        // 长图不支持
    ...
    func imagePanRemoveAnimation(_ isTop: Bool,imageview: UIView) {
        ...
        if isTop {
            ...
            UIView.animate(withDuration: duration) {
                imageview.frame = CGRect(x: 0, y: ScreenH, width: imgW, height: imgH)
            }
